...
```bash
# Asymmetric unit (what you get by default — NOT necessarily the biological complex)
...
curl -s "https://files.rcsb.org/download/6LFO-assembly1.cif.gz" | gunzip -c >6LFO-assembly1.cif
```
For most analysis (e.g. protein–protein interfaces, oligomeric state), use the **biological assembly** so the structure reflects the physiologically relevant multimer rather than the asymmetric unit — see below.

### Asymmetric Units, Biological Assemblies and Crystal Contacts

Contact analysis tools analyse exactly the coordinates given to them. Arpeggio, PyMOL and ChimeraX do **not** generate crystallographic symmetry mates, and no tool knows which chain–chain contacts are biologically meaningful. For any crystal structure of a complex, resolve this **before** analysing interfaces.

- The default download is the **asymmetric unit** (ASU) — the deposited coordinates. It may be the biological assembly, a *fraction* of it (the rest generated by symmetry), or *several* copies of it, or chains that form no complex at all.
- A **biological assembly** is the annotated physiological oligomer (mmCIF `_pdbx_struct_assembly*`, PDB `REMARK 350`). An entry can define more than one.
- A **crystal contact** exists only in the lattice. Two chains sitting next to each other in the deposited file are not necessarily a biological complex.

**Minimum checks before analysing a complex:**

```bash
# 1. What assemblies exist, and does the ASU already contain assembly 1?
...
print(a['oligomeric_details'], a['oligomeric_count'], a['details']); print(d['pdbx_struct_assembly_gen'])"
# oper_expression == '1'  -> deposited coordinates ARE the assembly
# anything else (e.g. '1,2') -> symmetry operators must be applied

# 2. Get the expanded assembly (mmCIF, single model, distinct chain IDs)
curl -s "https://files.rcsb.org/download/1HHO-assembly1.cif.gz" | gunzip -c >data/1HHO-assembly1.cif

#    ...or expand locally from the ASU
gemmi convert --assembly=1 data/1HHO.cif data/1HHO-asm1.cif

# 3. Verify the chain count matches oligomeric_count, and note the NEW chain IDs
gemmi residues -ss data/1HHO-assembly1.cif
```

> [!WARNING]
> Legacy `.pdb1`/`.pdb2` assembly files from RCSB put each symmetry copy in a **separate `MODEL` record with duplicate chain IDs**. Arpeggio and most other tools read only model 1, so you silently analyse the ASU. Check `grep -c "^MODEL" file.pdb1`; prefer `-assembly1.cif.gz` or `gemmi convert --assembly=1`.

Expansion **renames chains** (`A`, `B`, `A-2`, `B-2` for RCSB mmCIF assemblies; `A1`, `B1`, `A2`, `B2` for `gemmi convert`), so `--chains` and `-s` arguments written for the ASU must be updated.

To distinguish crystal contacts from real interfaces: an interface is biological if it lies within an annotated assembly. RCSB pre-computes per-interface buried area and core residue counts, and `gemmi contact` (symmetry-aware by default) can list contacts to symmetry mates with `--ignore=4`.

- **Reference:** [references/biological-assemblies.md](references/biological-assemblies.md) — terms, worked examples of each ASU/assembly relationship, interface metrics via the RCSB API, heuristics for judging biological relevance, PISA/EPPIC/ProtCID, and gemmi symmetry recipes.
...
Arpeggio's `-s` / `--selection` flag uses this syntax (fields can be left empty, but all three `/` separators are required):
...
# Select an entire chain (note the trailing empty atom field — `-s /A/` is rejected)
-s /A//
...
```bash
# Check the assembly annotation FIRST (see "Asymmetric Units, Biological Assemblies...")
curl -s "https://data.rcsb.org/rest/v1/core/assembly/1HHO/1" \
  | python3 -c "import sys,json; d=json.load(sys.stdin); a=d['pdbx_struct_assembly']; \
print(a['oligomeric_details'], a['details']); print(d['pdbx_struct_assembly_gen'])"
# -> tetrameric ... oper_expression '1,2': the deposited file holds only half the tetramer

# Fetch the biological assembly (not the asymmetric unit)
curl -s "https://files.rcsb.org/download/1HHO-assembly1.cif.gz" | gunzip -c >data/1hho-asm1.cif
gemmi residues -c data/1hho-asm1.cif    # -> A, B, A-2, B-2 (4 polymer chains, as expected)

# All inter-chain contacts in the assembly
python scripts/arpeggio_contact_analyser.py data/1hho-asm1.cif \
  --interchain-only -f tsv -o results/1hho_interfaces.tsv

# Contacts involving chains A or B-2 only
python scripts/arpeggio_contact_analyser.py data/1hho-asm1.cif \
  --chains A,B-2 --interchain-only \
  -f tsv -o results/1hho_A_B2.tsv

# Or use a selection to focus arpeggio on one chain
python scripts/arpeggio_contact_analyser.py data/1hho-asm1.cif \
  -s /A// --interchain-only \
  -f tsv -o results/1hho_A_interface.tsv
```
...
> [!NOTE]
> `--chains A,B` keeps contacts involving **either** chain, not only the A–B pair. With more than two chains (common after assembly expansion), filter the pair explicitly, e.g. `awk -F'\t' 'NR==1 || ($2=="A" && $6=="B") || ($2=="B" && $6=="A")'`.
...
└── references/
    ...
    ├── biological-assemblies.md           # ASU vs assembly, symmetry mates, crystal contacts
    ├── gemmi.md                           # gemmi CLI and Python API (conversion, symmetry, contacts)
...
- **`Invalid selector: /A/`**: chain-only selections need all three separators — use `-s /A//`
- **No interface found in a complex**: the interface may only exist after assembly expansion — check `_pdbx_struct_assembly_gen.oper_expression` (see [references/biological-assemblies.md](references/biological-assemblies.md))
- **Interface found but not biological**: chains adjacent in the asymmetric unit may be a crystal contact — confirm they are in the same annotated assembly
- **Assembly file gives ASU-only results**: `.pdb1` files split copies across `MODEL` records and only model 1 is read — use `-assembly1.cif.gz` or `gemmi convert --assembly=1`
- **Unknown chain after expansion**: expansion renames chains (`A-2`, `A1`, `C`…) — run `gemmi residues -c` to list them
