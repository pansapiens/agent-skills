#!/usr/bin/env python
"""Run RCSB PDB Data API GraphQL queries. See references/rcsb-data-api.md."""
...
def graphql(query: str, variables: Optional[dict] = None) -> dict:
    ...
    r = requests.post(DATA_GRAPHQL, json=payload, timeout=30)
    r.raise_for_status()
    return r.json()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run RCSB Data API GraphQL query. Query from stdin or --query."
    )
    parser.add_argument(
        "-q", "--query",
        type=str,
