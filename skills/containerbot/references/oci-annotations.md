...
- **org.opencontainers.image.usage** the usage of the image, eg `docker run -it --gpus all --rm <image> <command>` (this is an out-of-spec annotation, but often included in the wild, so we can use it)
