"""
annotation_compare.py

Reusable pipeline for comparing Ensembl vs NCBI RefSeq genome annotations
(GTF/GFF3) for the *same* assembly. Given a GenBank accession (GCA_...) and
its RefSeq counterpart (GCF_...), this module will:

  1. Resolve the correct Ensembl FTP GTF/GFF3 files for that assembly
     (walking Ensembl releases via the REST API + FTP listing).
  2. Resolve the correct NCBI RefSeq FTP GTF/GFF3 files for that assembly.
  3. Download and parse both into gene/transcript/exon-level tables.
  4. Build a chromosome/scaffold name-mapping between the two naming
     schemes using the NCBI assembly report.
  5. Compute per-source summary statistics (gene/transcript/exon counts,
     biotype composition, isoform complexity).
  6. Cross-match genes by genomic-interval overlap and quantify
     concordance/discordance in biotype calls.

Designed to be driven either interactively (python/repl tool) or from the
companion `annotation_compare.qmd` Quarto notebook, which is parameterized
by `gca_accession` / `gcf_accession` so the whole analysis can be re-run for
any other organism with both an Ensembl and an NCBI RefSeq annotation.

No third-party dependencies beyond `requests` and `pandas`.
"""
...
def _get_with_retry(session: requests.Session, url: str, *, tries: int = 5, backoff: float = 3.0, **kwargs):
    """Ensembl's REST API (EBI infra) intermittently 500s under no apparent
    load; a short retry-with-backoff clears it almost every time."""
    last_exc = None
...
# Biotype terms -> a small set of categories that are comparable across
# annotation providers (Ensembl and NCBI RefSeq use different vocabularies
...
def normalize_biotype(biotype: Optional[str]) -> str:
...
# --------------------------------------------------------------------------
# Resolution: find the right files on Ensembl FTP / NCBI FTP for a given
# assembly accession pair.
# --------------------------------------------------------------------------

@dataclass
class AnnotationSource:
    provider: str            # "ensembl" | "ncbi"
    species: str = ""
    assembly_name: str = ""
    release: str = ""
    gtf_url: str = ""
...
def resolve_ensembl(gca_accession: str, session: Optional[requests.Session] = None) -> AnnotationSource:
    """Resolve the Ensembl (vertebrates) GTF/GFF3 URLs for a GCA accession.

    Strategy: ask the Ensembl REST API which species/assembly this GCA
    corresponds to, then walk main-site releases (newest first) looking for
    a GTF/GFF3 file matching that species + assembly_default. Falls back to
    the Ensembl Rapid Release FTP tree (organized directly by GCA accession)
    if the assembly isn't (yet) on the main site.
    """
    s = session or requests.Session()
    r = _get_with_retry(s, f"{ENSEMBL_REST}/info/genomes/assembly/{gca_accession}",
                         params={"content-type": "application/json"})
    info = r.json()
    species_prod_name = info["name"]            # e.g. "ovis_aries"
    url_name = info.get("url_name", species_prod_name.capitalize())
    assembly_default = info["assembly_default"]  # e.g. "ARS-UI_Ramb_v3.0"

    # Find available releases on the FTP (descending) and probe for files.
    idx = _get_with_retry(s, f"{ENSEMBL_FTP}/").text
    ...
    for rel in releases:
        ...
        head = s.head(gtf_url, timeout=30)
        if head.status_code == 200:
            return AnnotationSource(provider="ensembl", species=species_prod_name,
                                     assembly_name=assembly_default, release=str(rel),
                                     gtf_url=gtf_url, gff3_url=gff3_url)

    # Fall back: Ensembl Rapid Release (organized by GCA accession directly).
    rapid_dir = f"{ENSEMBL_FTP}/rapid-release/species/{url_name}/{gca_accession}/"
    rr = s.get(rapid_dir, timeout=30)
...
def resolve_ncbi(gcf_accession: str, session: Optional[requests.Session] = None) -> AnnotationSource:
    ...
    idx = s.get(triplet_dir, timeout=30)
    ...
    return AnnotationSource(
        provider="ncbi", assembly_name=assembly_name, release=asm_dir_name,
...
def parse_gtf(path: str, attr_keys=GTF_ATTR_KEYS) -> pd.DataFrame:
...
def parse_ncbi_assembly_report(path: str) -> pd.DataFrame:
    with open(path) as f:
        lines = f.readlines()
...
def ensembl_name_for_report_row(row, ensembl_seq_names: set) -> Optional[str]:
    """Best-effort mapping from an NCBI assembly-report row to the seqid
    Ensembl would use for the same physical sequence (chromosome numbers/
    letters for assembled molecules, GenBank contig accession for
    ...
    if gb != "na" and gb in ensembl_seq_names:
        return gb
    # last resort: raw Sequence-Name or GenBank accession without ".1" suffix games
    if sn in ensembl_seq_names:
        return sn
    return None


def build_chrom_mapping(report: pd.DataFrame, ensembl_seq_names: set) -> dict:
    report = report.copy()
...
# --------------------------------------------------------------------------
# Gene/transcript/exon table construction & summary stats
# --------------------------------------------------------------------------
...
def build_tables(gtf_df: pd.DataFrame, label: str) -> ParsedAnnotation:
    ...
    return ParsedAnnotation(genes=genes, transcripts=transcripts, exons=exons, label=label)


def summary_stats(ann: ParsedAnnotation) -> dict:
...
# --------------------------------------------------------------------------
# Cross-annotation locus overlap matching
# --------------------------------------------------------------------------

def overlap_pairs(gA: pd.DataFrame, gB: pd.DataFrame, chrom_col: str = "chrom_unified") -> pd.DataFrame:
...
def classify_gene_fates(genes_a: pd.DataFrame, genes_b: pd.DataFrame, pairs: pd.DataFrame) -> pd.Series:
    """For each gene in genes_a, classify its relationship to genes_b as
    'concordant' (overlaps a same-biotype gene), 'discordant' (overlaps a
    gene of a different biotype only), or 'unique' (no overlap at all)."""
...
def run_pipeline(gca_accession: str, gcf_accession: str, workdir: str = "data", overwrite: bool = False):
    """Download + parse + cross-match Ensembl vs NCBI RefSeq annotations
    for one assembly. Returns a dict with parsed tables, summary stats, and
    overlap results -- everything the report/plots need."""
    os.makedirs(workdir, exist_ok=True)
    session = requests.Session()

    ens_src = resolve_ensembl(gca_accession, session)
    ncbi_src = resolve_ncbi(gcf_accession, session)
    ...
    ens_fate = classify_gene_fates(ens_ann.genes, ncbi_ann.genes, pairs)
    ncbi_fate = classify_gene_fates(
        ncbi_ann.genes, ens_ann.genes,
        pairs.rename(columns={"a_idx": "b_idx", "b_idx": "a_idx"})[["a_idx", "b_idx"]],
    )
    ens_ann.genes["fate_vs_other"] = ens_fate
    ncbi_ann.genes["fate_vs_other"] = ncbi_fate

    return {
        "ensembl_source": ens_src,
        "ncbi_source": ncbi_src,
        "ensembl": ens_ann,
        "ncbi": ncbi_ann,
...
# --------------------------------------------------------------------------
# Locus-diagram helpers (worked-example figures for the report)
#
# A table of gene-ID pairs tells the reader THAT two providers disagree at a
# locus; these helpers draw the actual gene/transcript models to scale,
# colored by biotype, so the report shows WHAT the disagreement looks like.
# Used by annotation_compare.qmd's "Worked examples" section.
# --------------------------------------------------------------------------
...
def _pair_frame_with_biotypes(result):
    """result: the dict returned by run_pipeline(). Note ens_ann/ncbi_ann are
    ParsedAnnotation dataclasses here (attribute access: .genes, not ["genes"])."""
...
def pick_example_region_locus(result, biotype_a="lncRNA", biotype_b="protein_coding",
    ...
    """Auto-select a real 'nested annotation' example locus (an NCBI gene
    surrounded/overlapped by a moderate cluster of disagreeing Ensembl
    calls) from THIS organism's own data. Returns a dict of region bounds +
    gene-table slices, or None if no locus in the size range exists."""
    pairs = _pair_frame_with_biotypes(result)
    ...
    if len(counts) == 0:
        return None
    target_ncbi_id = counts.sort_values().index[len(counts) // 2]
...
def pick_example_isoform_locus(result, min_tx=2, max_tx=6, overlap_frac_min=0.8):
    ...
    differing transcript count on each side. Returns a dict of gene ids/rows
...
if __name__ == "__main__":
    ...
    p.add_argument("gca", help="GenBank assembly accession, e.g. GCA_016772045.2")
    p.add_argument("gcf", help="RefSeq assembly accession, e.g. GCF_016772045.2")
    p.add_argument("--workdir", default="data")
    args = p.parse_args()

    result = run_pipeline(args.gca, args.gcf, workdir=args.workdir)
    print(json.dumps({"ensembl": result["ensembl_stats"], "ncbi": result["ncbi_stats"]}, indent=2, default=str))
