   ;***************************************************************
   ;
   ;  Multisprite: 9 Objects With Coordinates and Collision
   ;
   ;
   ...
   ;
   ;  This program builds on the original example by adding
   ;  collision detection to check if player0 overlaps another
   ;  sprite, using bounding box checks to identify which sprite is
   ;  touched. When a collision occurs, the background color changes
   ;  to indicate the overlapped sprite, and the touched sprite snaps
   ;  back to its original position. Try moving player0 to collide
   ;  with multiple sprites near each other on one side of the screen,
   ;  sending each back one at a time with a single move.

   ;  If all sprites are set to the default width (8 pixels), hardware
   ;  collision detection is used for precise pixel-to-pixel checks.
   ;  If any sprite (player0 through player5) has a wider width (16 or
   ;  32 pixels), the program switches to bounding box checks to handle
   ;  the larger sprite sizes accurately.
   ;  
   ;```````````````````````````````````````````````````````````````
   ;
...
__Skip_Movement
   ...
   ;
   ;```````````````````````````````````````````````````````````````
   ;  Jumps to collision check if all sprites are default size.
   ;
   if !_Sprite_Size0 && !_Sprite_Size1 && !_Sprite_Size2 && !_Sprite_Size3 && !_Sprite_Size4 && !_Sprite_Size5 then goto __Collision_Check
...
__Collision_Check
   ...
   ;```````````````````````````````````````````````````````````````
   ;  Skips this section if no collision between p0 and p1.
   ;
...
__Skip_C_Check
   ...
   ;```````````````````````````````````````````````````````````````
   ;  Bounding box checks for p0 with p1 through p5.
   ;
   temp4 = 8
   temp6 = _Data_8_16_32[_Sprite_Size0] : if _Sprite_Size0 then temp4 = 10

   temp5 = _Data_8_16_32[_Sprite_Size1] : temp3 = 7
   if _Sprite_Size1 then temp5 = temp5 - 1 : temp3 = 6 : if _Sprite_Size0 then temp4 = 9
   if player1x <= player0x + temp6 + temp3 && player1x + temp5 - 1 >= player0x + temp4 && player1y <= player0y + player1height - 5 && player1y + player1height >= player0y - player0height + 8 then goto __p1box

   temp5 = _Data_8_16_32[_Sprite_Size2] : temp3 = 7
   if _Sprite_Size2 then temp5 = temp5 - 1 : temp3 = 6 : if _Sprite_Size0 then temp4 = 9
   if player2x <= player0x + temp6 + temp3 && player2x + temp5 - 1 >= player0x + temp4 && player2y <= player0y + player2height - 5 && player2y + player2height >= player0y - player0height + 8 then goto __p2box

   temp5 = _Data_8_16_32[_Sprite_Size3] : temp3 = 7
   if _Sprite_Size3 then temp5 = temp5 - 1 : temp3 = 6 : if _Sprite_Size0 then temp4 = 9
   if player3x <= player0x + temp6 + temp3 && player3x + temp5 - 1 >= player0x + temp4 && player3y <= player0y + player3height - 5 && player3y + player3height >= player0y - player0height + 8 then goto __p3box

   temp5 = _Data_8_16_32[_Sprite_Size4] : temp3 = 7
   if _Sprite_Size4 then temp5 = temp5 - 1 : temp3 = 6 : if _Sprite_Size0 then temp4 = 9
   if player4x <= player0x + temp6 + temp3 && player4x + temp5 - 1 >= player0x + temp4 && player4y <= player0y + player4height - 5 && player4y + player4height >= player0y - player0height + 8 then goto __p4box

   temp5 = _Data_8_16_32[_Sprite_Size5] : temp3 = 7
   if _Sprite_Size5 then temp5 = temp5 - 1 : temp3 = 6 : if _Sprite_Size0 then temp4 = 9
   if player5x <= player0x + temp6 + temp3 && player5x + temp5 - 1 >= player0x + temp4 && player5y <= player0y + player5height - 5 && player5y + player5height >= player0y - player0height + 8 then goto __p5box

   goto __Skip_Collision

__p1box player1x = 85 : player1y = 70 : COLUBK = $12 : goto __Skip_Collision
__p2box player2x = 85 : player2y = 55 : COLUBK = $32 : goto __Skip_Collision
__p3box player3x = 85 : player3y = 40 : COLUBK = $62 : goto __Skip_Collision
__p4box player4x = 85 : player4y = 25 : COLUBK = $82 : goto __Skip_Collision
__p5box player5x = 85 : player5y = 10 : COLUBK = $C2
...
__Skip_Collision
   ...
   ;***************************************************************
   ;
   ;  Sprite width data for bounding box checks.
   ;
   data _Data_8_16_32
   8, 17, 33
end
